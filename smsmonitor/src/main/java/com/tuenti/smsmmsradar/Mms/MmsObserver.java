...
import com.tuenti.smsmmsradar.Mms.MmsCursorParser;
import com.tuenti.smsmmsradar.SmsMmsRadar;
...
public class MmsObserver extends ContentObserver {
    ...
    private static final String PROTOCOL_COLUM_NAME = "msg_box";
...
//        Telephony.Mms.Sent.CONTENT_URI;
    ...
    private void processMms(Cursor cursor) {
        ...
        try {
            int protocol = cursor.getInt(cursor.getColumnIndex(PROTOCOL_COLUM_NAME));
            mmsCursor = getMmsCursor(protocol);
            Mms mms = parseMms(mmsCursor);
    ...
    private void notifyMmsListener(Mms mms) {
        if (mms != null && SmsMmsRadar.mmsListener != null) {
            if (SmsType.SENT == mms.getType()) {
    ...
    private Mms parseMms(Cursor cursor) {
