...
public class MmsCursorParser
{
    ...
    Mms parse(Cursor cursor, ContentResolver resolver) {

        if (!canHandleCursor(cursor) || !cursor.moveToNext()) {
            return null;
        }

        Mms mmsParsed = extractMmsInfoFromCursor(cursor, resolver);
    ...
    private Mms extractMmsInfoFromCursor(Cursor cursor, ContentResolver resolver) {
//        MessageDbUtility
